    ...
    def __init__(
        ...
        self.pts_linears = nn.ModuleList(
            ...
            + [
                ...
                else DenseLayer(W + input_ch, W, activation="relu")
                for i in range(D - 1)
            ]
        )
        ...
        self.views_linears = nn.ModuleList(
            [DenseLayer(input_ch_views + W, W // 2, activation="relu")]
        )
...
class SineLayer(nn.Module):
    def __init__(
        self, in_dims: int, out_dims: int, is_first: bool = False, w0: float = 30.0
    ):
        super().__init__()
        ...
        if self.is_first:
            nn.init.uniform_(self.linear.weight, -1 / in_dims, 1 / in_dims)
        else:
            nn.init.uniform_(
                self.linear.weight,
                -np.sqrt(6 / in_dims) / self.w0,
                np.sqrt(6 / in_dims) / self.w0,
            )
...
class SirenNeRF(nn.Module):
    def __init__(
        ...
        self.pts_linears = nn.ModuleList(
            [SineLayer(input_ch, W, is_first=True, w0=self.w0)]
            + [
                SineLayer(W, W, w0=self.w0)
                if i not in self.skips
                else SineLayer(W + input_ch, W, w0=self.w0)
                for i in range(D - 1)
            ]
        )

        ### Implementation according to the official code release (https://github.com/bmild/nerf/blob/master/run_nerf_helpers.py#L104-L105)
        self.initial_views_linears = SineLayer(
            input_ch_views, W // 2, is_first=True, w0=self.w0
        )
        self.views_linears = nn.ModuleList([SineLayer(W // 2 + W, W // 2)])
    ...
    def forward(self, x):
        ...
        if self.use_viewdirs:
            alpha = self.alpha_linear(h)
            feature = self.feature_linear(h)
            h = torch.cat([feature, self.initial_views_linears(input_views)], -1)
